...
| :--------------------------- | :----------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `keyName`                    | string                         | Puede ser la clave con nombre (es decir, `LSP3Profile`, `LSP12IssuedAssetsMap:<address>`) o la clave hash (con o sin el prefijo `0x`, es decir, `0x5ef...` o `5ef...`). |
| `dynamicKeyParts` (opcional) | string or <br/> string[&nbsp;] | Si `keyName` es una clave dinámica, las partes dinámicas de `keyName` que se utilizarán para codificar la clave.                                                        |
| `value`                      | string or <br/> string[&nbsp;] | El valor que debe descodificarse. Puede ser una cadena, una matriz de cadenas o un JSON...                                                                               |
...
- `dynamicKeyParts`: string o string[&nbsp;] que contiene las variables que necesitan ser codificadas.

##### 2. `schemas` - Conjunto de objetos (opcional)

Un conjunto de objetos extra [LSP-2 EsquemaJSONERC725Y] que pueden utilizarse para encontrar el esquema. Si se llama en una instancia, es opcional y se concatenará con el esquema proporcionado en la instanciación.

#### Devuelve

| Nombre        | Tipo               | Descripción                                                                          |
| :------------ | :----------------- | :----------------------------------------------------------------------------------- |
| `decodedData` | Object or Array    | Los datos descodificados tal y como se definen y esperan en los siguientes esquemas. |
...
| Nombre               | Tipo   | Descripción                                                                                                   |
| :------------------- | :----- | :------------------------------------------------------------------------------------------------------------ |
| `decodedPermissions` | Object | Un objeto que especifica si los permisos LSP6 por defecto se incluyen en la cadena hexadecimal proporcionada. |
...
| :--------------------------- | :--------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `keyName`                    | string                                         | Puede ser la clave con nombre (es decir, `LSP3Profile`, `LSP12IssuedAssetsMap:<address>`) o la clave hash (con o sin el prefijo `0x`, es decir, `0x5ef...` o `5ef...`). |
| `dynamicKeyParts` (optional) | string or <br/> string[&nbsp;]                 | Las partes dinámicas del `keyName` que se utilizarán para cifrar la clave.                                                                                       |
| `value`                      | string or <br/> string[&nbsp;] <br/> JSON todo | El valor que debe codificarse. Puede ser una cadena, un conjunto de cadenas o un JSON...                                                                               |
...
| Nombre        | Tipo   | Descripción                                                                                                                                                                                                |
| :------------ | :----- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `encodedData` | Object | Un objeto que contiene las claves y valores cifrados de acuerdo con el [LSP2 Esquema JSON ERC725Y](https://github.com/lukso-network/LIPs/blob/main/LSPs/LSP-2-ERC725YJSONSchema.md) de los datos que se han pasado. |
...
| Nombre           | Tipo   | Descripción                                             |
| :--------------- | :----- | :------------------------------------------------------ |
| `encodedKeyName` | string | El hash keccak256 del nombre de la clave proporcionada. |
...
Analiza una clave hasheada o una lista de claves hasheadas e intentará devolver su correspondiente objeto [LSP2 EsquemaJSONERC725Y](https://github.com/lukso-network/LIPs/blob/main/LSPs/LSP-2-ERC725YJSONSchema.md). Además, buscará la clave correspondiente dentro de los esquemas:
...
Un conjunto de objetos extra [LSP-2 EsquemaJSONERC725Y] que pueden ser utilizados para encontrar el esquema.
...
[esquema JSON erc725 lsp-2]: https://github.com/lukso-network/LIPs/blob/main/LSPs/LSP-2-ERC725YJSONSchema.md
